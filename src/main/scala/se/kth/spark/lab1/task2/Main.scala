...
object Main {
  def main(args: Array[String]) {
   ...
   //Step6: shift all labels by the value of minimum label such that the value of the smallest becomes 0 (use our DoubleUDF) -------- 
    //In order to find the minimum, we sort the result of the previous transformation and take the 4th value (year) of the first row. 
    val min_year = v2d_resp.sort("label", "tokens_vector").first().get(4)
    println("Minimum year: " + min_year)
